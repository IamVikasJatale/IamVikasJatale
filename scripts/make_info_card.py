...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">vikas@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14.5" font-weight="700">'
                 f'<tspan fill="{GREEN}">vikas</tspan><tspan fill="{MUTED}">@</tspan>'
                 f'<tspan fill="{ACCENT}">github</tspan></text>'
                 f'<rect x="{KEY_X+118}" y="{y-4:.1f}" width="{W-PAD-(KEY_X+118)}" height="1.5" fill="url(#hostGrad)"/>')
    elif kind == "sec":
        title = esc(row[1])
